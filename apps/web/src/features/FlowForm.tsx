      ...
      <form onSubmit={submit} className="space-y-4 text-sm">
        <Field label="어디로 나가는 돈인가요?">
          <input required value={merchant} onChange={(e) => setMerchant(e.target.value)}
            className="kb-input" placeholder="예: 넷플릭스" />
        </Field>

        {similar.length > 0 && (
          <div className="rounded-2xl bg-kakao/30 px-4 py-3 text-xs font-medium text-kakao-dark">
        ...
        <Field label="얼마?">
          <div className="flex items-center gap-2">
            <input value={amount} onChange={(e) => setAmount(e.target.value)}
              ...
              className="kb-input flex-1 text-right disabled:opacity-40" />
            <span className="text-dim font-medium">원</span>
          </div>
          <label className="mt-2 flex items-center gap-2 text-xs text-dim">
            <input type="checkbox" checked={isVariable} onChange={(e) => setIsVariable(e.target.checked)}
              className="h-4 w-4 rounded accent-kakao" />
            매달 달라요 (변동 금액)
          </label>
        </Field>
        ...
        <Field label="언제? (매달 N일)">
          <input required value={day} onChange={(e) => setDay(e.target.value)}
            inputMode="numeric"
            className="kb-input w-28 text-center" />
        </Field>

        <Field label="어디서 빠져요?">
          <div className="mb-2 flex gap-2">
            <button type="button" onClick={() => setVia('card')}
              className={`flex-1 rounded-2xl border-2 py-2.5 text-sm font-semibold transition-colors ${via === 'card' ? 'border-kakao bg-kakao text-kakao-dark' : 'border-line text-dim'}`}>💳 카드로</button>
            <button type="button" onClick={() => setVia('account')}
              className={`flex-1 rounded-2xl border-2 py-2.5 text-sm font-semibold transition-colors ${via === 'account' ? 'border-kakao bg-kakao text-kakao-dark' : 'border-line text-dim'}`}>🏦 자동이체</button>
          </div>
          {via === 'card' ? (
          ...
          {noSource && (
            <p className="mt-1.5 rounded-2xl bg-warn/10 px-3 py-2 text-xs font-medium text-warn">먼저 {via === 'card' ? '카드' : '계좌'}를 등록해야 합니다.</p>
          )}
        </Field>
        ...
        <Field label="메모 (선택)">
          <input value={notes} onChange={(e) => setNotes(e.target.value)}
            className="kb-input" placeholder="기억해두고 싶은 메모" />
        </Field>

        <label className="flex items-center gap-2 rounded-2xl bg-bg px-4 py-3 text-xs text-dim">
        ...
        <div className="flex gap-2 pt-1">
          {isEdit && onDelete && (
            <button type="button" onClick={onDelete}
              className="kb-btn-danger px-4 py-3">해지</button>
          )}
          <button disabled={busy || noSource} type="submit"
            className="kb-btn flex-1 py-3">
            {busy ? '저장 중...' : isEdit ? '변경 사항 확인' : '저장'}
          </button>
        </div>
        {err && <p className="rounded-2xl bg-bad/10 px-3 py-2 text-xs text-bad">{err}</p>}
      </form>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="mb-1.5 block text-xs font-semibold text-dim">{label}</label>
      {children}
    </div>
  );
}
