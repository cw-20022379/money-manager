...
이 테스트가 검증하는 것:
  payment_flows.schedule_day 기반으로 월별 현금흐름 캘린더를 렌더링하는 기능을 검증한다.
  "이번 달 어느 날에 얼마가 나가는가"를 시각적으로 확인하는 핵심 UX다.

  검증 항목:
  1. 캘린더 그리드: 7열(요일) × N행 그리드가 올바르게 렌더링되는지.
  2. 날짜별 금액 표시: schedule_day가 5·15·25일인 정기지출이 해당 셀에 표시되는지.
  3. 금액 톤(색상 테두리) 검증:
     - 소액(5,000원) = teal 톤 (border-teal)
     - 중간(60,000원) = warn 톤 (border-warn)
     - 고액(130,000원) = bad 톤 (border-bad)
     CATEGORY_COLOR와는 다른 "부담 정도" 기반 색상 스케일.
  4. 셀 클릭 → 상세 패널: 날짜 셀 클릭 시 "일 빠질 돈" 상세 패널이 열리는지.
  5. 상세 항목 클릭 내비게이션: 패널 내 항목 클릭 시 /list + 편집 모달 자동 오픈.
  6. 월 이동: "›" 버튼으로 다음 달, "오늘" 버튼으로 이번 달 복귀.

데이터 전략:
  의도적으로 다른 날(5/15/25일)에 다른 금액(소/중/대)의 정기지출 3건을 등록해
  캘린더의 날짜 배치와 색상 톤이 올바르게 동작하는지 한 번에 확인한다.
